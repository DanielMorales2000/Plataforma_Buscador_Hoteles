...
<body class="main-layout about-page">
    ...
    <div class="row" style="position: relative; left: 30px; top: 50px">
        <form method="POST">
            <div class="col-sm-12">
                <label> Buscar por No. de identificacion de empleado:</label>
                <input class="contactus" type="text" name="buscar_emp" placeholder="Inserte documento de identificacion del empleado">
            </div>
            <div class="form-group">				
                <input type="submit" name="buscar_empleados" class="send" value="Buscar"><br />
            </div>
        </form>
        <br><br>
        <br>
        ...
        ?>
        <form action="../frontend/crear_empleados" style="position: relative; left: -350px; top: 105px">
            <input type="submit" class="send" value="Crear Empleados"><br />
        </form>
        <?php 
            }
        ?>
        ...
        <table class="table table-bordered table-responsive" style="position: relative; left: 30px; top: 50px">
            <tr>
                ...
                <td width="100" align="center">Cuenta</td>
                <td width="300" align="center">Correo</td>
                <td width="100" align="center">Tipo de Documento</td>
            ...
            <?php
                ...
                $id_c_corp = $_SESSION['id_c_corp'];
                $doc_empleado = null;
                if (isset($_POST['buscar_empleados']) && $_POST['buscar_emp'] != NULL) {
                    $doc_empleado = $_POST['buscar_emp'];
                    $consulta = "[dbo].[PA_LISTAR_EMPLEADOS]
                    @ID_HOTEL = '$id_c_corp',
                    @DOCUMENTO = '$doc_empleado'";
                }
                else {
                    $consulta = "[dbo].[PA_LISTAR_EMPLEADOS]
                    @ID_HOTEL = '$id_c_corp'";
                ...
                while($fila = sqlsrv_fetch_array($ejecutar)){
                    ...
                    $CORREO = $fila['CORREO'];
                    $SIGLA_TIPO_DOCUMENTO = $fila['SIGLA_TIPO_DOCUMENTO'];
                    $DOCUMENTO = $fila['DOCUMENTO'];
                    $SEXO = $fila['SEXO'];
                    $ROL = $fila['ROL'];
                    $NOMBRE_H = $fila['NOMBRE_HOTEL'];
                    $i++;
                ?>
                ...
                <td><?php echo $NOMBRE_SUB_CUENTA; ?></td>
                <td><?php echo $CORREO; ?></td>
                <td><?php echo $SIGLA_TIPO_DOCUMENTO; ?></td>
                <td><?php echo $DOCUMENTO; ?></td>
                <td><?php echo $SEXO; ?></td>
                <td><?php echo $ROL; ?></td>
                <td><?php echo $NOMBRE_H; ?></td>
                <td><a 
                <?php 
                    if ($_SESSION["rol_cc"] == 1) {
